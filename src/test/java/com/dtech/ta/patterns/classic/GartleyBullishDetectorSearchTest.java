...
/**
 * Unit tests for bullish Gartley detector search and pivot management.
 */
class GartleyBullishDetectorSearchTest {
    ...
    void testCleanGartleyFind() {
        ...
        // At least verify detection doesn't crash and pivots are valid if any pattern found
    ...
    void testFindAllReturnsMultiplePatterns() {
        ...
        // May find 0 or more patterns depending on pivot arrangement
        patterns.forEach(p -> {
            assertEquals(Direction.BULLISH, p.direction());
            assertEquals(5, p.pivots().size());
        });
    ...
    void testLookbackParameterRespected() {
        ...
        // Both should be valid (may or may not find patterns)
