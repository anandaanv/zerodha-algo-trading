...
/**
 * Detector for Bearish Butterfly harmonic pattern.
 * 5 pivots: X (HIGH) -> A (LOW) -> B (HIGH) -> C (LOW) -> D (HIGH, current).
 * b_retrace must be == 0.786; c_retrace must be in [0.382, 0.886].
 * Terminal point: 1.27 XA extension.
 * Perfect Butterfly: b_retrace == 0.786 AND c_retrace in [0.5, 0.886].
 * Reference: docs/spec-classic-patterns-port.md (Butterfly Bearish section)
 */
public class ButterflyBearishDetector implements ClassicPatternDetector<ButterflyPattern> {

    private static final double EPSILON = 1e-6;

    @Override
    public Optional<ButterflyPattern> findLatest(BarSeries series, List<PivotPoint> pivots, int lookbackBars) {
        List<ButterflyPattern> all = findAll(series, pivots, lookbackBars);
        return all.isEmpty() ? Optional.empty() : Optional.of(all.get(all.size() - 1));
    }

    @Override
    public List<ButterflyPattern> findAll(BarSeries series, List<PivotPoint> pivots, int lookbackBars) {
        List<ButterflyPattern> patterns = new ArrayList<>();
    ...
    /**
     * Search for bearish Butterfly patterns using the X := highest-pivot-in-series loop.
     */
    private List<ButterflyPattern> searchBearish(BarSeries series, List<PivotPoint> pivots,
                                                 int dBarIndex, double dPrice,
                                                 List<ButterflyPattern> patterns) {
        if (pivots.isEmpty()) {
            return patterns;
        }
        ...
        for (int xIdx = 0; xIdx < pivots.size(); xIdx++) {
            ...
            boolean isPerfect = Math.abs(bRetrace - 0.786) < EPSILON &&
                               cRetrace >= 0.5 - EPSILON && cRetrace <= 0.886 + EPSILON;

            if (Math.abs(bRetrace - 0.786) > EPSILON ||
                cRetrace < 0.382 - EPSILON || cRetrace > 0.886 + EPSILON) {
                continue;
            }

            double xa127Ext = aPrice + xaDiff * 1.27;
            double terminalPoint = xa127Ext;
            ...
            ButterflyPattern.ButterflyKind kind = isPerfect ? ButterflyPattern.ButterflyKind.PERFECT :
                                                  ButterflyPattern.ButterflyKind.REGULAR;
    ...
    /**
     * Check if Butterfly is well-formed: lowest_xb == a, highest_ac == b, lowest_from_b == c.
     * Also: a not bar's high, b not bar's low, c not bar's high.
     */
