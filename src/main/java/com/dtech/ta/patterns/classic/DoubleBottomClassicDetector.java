...
/**
 * Detector for Double Bottom patterns (bullish reversal).
 * Walks 4-pivot windows (LOW-HIGH-LOW) plus current close, validates volume and ATR constraints.
 * Reference: docs/spec-classic-patterns-port.md (Double Bottom section)
 */
public class DoubleBottomClassicDetector implements ClassicPatternDetector<DoubleBottomPattern> {

    @Override
    public Optional<DoubleBottomPattern> findLatest(BarSeries series, List<PivotPoint> pivots, int lookbackBars) {
        List<DoubleBottomPattern> all = findAll(series, pivots, lookbackBars);
        return all.isEmpty() ? Optional.empty() : Optional.of(all.get(all.size() - 1));
    }

    @Override
    public List<DoubleBottomPattern> findAll(BarSeries series, List<PivotPoint> pivots, int lookbackBars) {
        List<DoubleBottomPattern> patterns = new ArrayList<>();
        ...
        for (int i = 0; i <= pivots.size() - 3; i++) {
            ...
            try {
                ...
                if (test(a.price(), b.price(), c.price(), dPrice, aVol, cVol, avgBar, atr)) {
                    ...
                    patternPivots.add(PivotPoint.of(series, cBarIndex, PivotType.HIGH));

                    DoubleBottomPattern pattern = new DoubleBottomPattern(
                        Direction.BULLISH,
                        patternPivots,
                        aVol,
                        cVol,
    ...
    /**
     * Test Double Bottom geometry: valleys at similar level, declining volume, neckline constraints, ATR distance.
     */
