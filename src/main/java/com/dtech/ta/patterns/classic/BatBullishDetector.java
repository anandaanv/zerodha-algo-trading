...
/**
 * Detector for Bullish BAT harmonic pattern.
 * 5 pivots: X (LOW) -> A (HIGH) -> B (LOW) -> C (HIGH) -> D (LOW, current).
 * b_retrace must be in {0.382, 0.5}; c_retrace must be in [0.382, 0.886].
 * Reference: docs/spec-classic-patterns-port.md (BAT Bullish section)
 */
public class BatBullishDetector implements ClassicPatternDetector<BatPattern> {
    ...
    /**
     * Search for bullish BAT patterns using the X := lowest-pivot-after-X advance loop.
     */
    private List<BatPattern> searchBullish(BarSeries series, List<PivotPoint> pivots,
    ...
    /**
     * Check if BAT is well-formed: highest_xb == a, lowest_ac == b, highest_from_b == c.
