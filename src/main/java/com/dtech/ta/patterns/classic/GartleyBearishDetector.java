...
/**
 * Detector for Bearish Gartley harmonic pattern.
 * 5 pivots: X (HIGH) -> A (LOW) -> B (HIGH) -> C (LOW) -> D (HIGH, current).
 * b_retrace must be == 0.618; c_retrace must be in [0.382, 0.886].
 * Terminal point: 0.786 XA extension.
 * Reference: docs/spec-classic-patterns-port.md (Gartley Bearish section)
 */
public class GartleyBearishDetector implements ClassicPatternDetector<GartleyPattern> {
    ...
    /**
     * Search for bearish Gartley patterns using the X := highest-pivot-after-X advance loop.
     */
    private List<GartleyPattern> searchBearish(BarSeries series, List<PivotPoint> pivots,
        ...
        while (true) {
            ...
            double xa786Ext = aPrice + xaDiff * 0.786;
            double terminalPoint = xa786Ext;
    ...
    /**
     * Check if Gartley is well-formed: lowest_xb == a, highest_ac == b, lowest_from_b == c.
