...
/**
 * Detector for Downtrend Line patterns (resistance lines).
 * Connects descending HIGH pivots such that price does not breach above the line.
 * Direction: BEARISH (resistance line).
 * Reference: docs/spec-classic-patterns-port.md (Trendline section)
 */
public class DowntrendLineDetector implements ClassicPatternDetector<TrendlinePattern> {
    ...
    public List<TrendlinePattern> findAll(BarSeries series, List<PivotPoint> pivots, int lookbackBars) {
        ...
        // Filter to HIGH pivots only and in recent lookback
        List<PivotPoint> highPivots = new ArrayList<>();
        int minEndBarIndex = Math.max(0, series.getEndIndex() - lookbackBars);

        for (PivotPoint p : pivots) {
            if (p.type() == PivotType.HIGH && p.barIndex() >= minEndBarIndex) {
                highPivots.add(p);
            }
        }
        ...
        // Try all pairs of HIGH pivots
        for (int i = 0; i < highPivots.size() - 1; i++) {
            for (int j = i + 1; j < highPivots.size(); j++) {
                PivotPoint a = highPivots.get(i);
                PivotPoint b = highPivots.get(j);
                ...
                for (int barIdx = a.barIndex(); barIdx <= b.barIndex(); barIdx++) {
                    double barHigh = series.getBar(barIdx).getHighPrice().doubleValue();
                    double lineValue = line.valueAt(barIdx);
                    double distanceAbove = barHigh - lineValue;

                    // Check for significant breach
                    if (distanceAbove > breachTolerance) {
                        breached = true;
                        break;
                    }

                    // Count touchpoint
                    if (Math.abs(distanceAbove) <= touchTolerance) {
                        touchpointCount++;
                    }
                }
