...
import { TradeAction, fetchTradeActions } from "./tradesApi";

export function TradeActionTimeline({ signalId }: { signalId: number }) {
  ...
  return (
    <div style={{ padding: "10px 20px", background: "#f8f9fa", borderLeft: "3px solid #ff6b35" }}>
        ...
        <tbody>
          {actions.map((a) => (
            <tr key={a.id} style={{ borderBottom: "1px solid #eee" }}>
              ...
              <td style={{ textAlign: "right", padding: "4px 8px", color: a.pnlPct && a.pnlPct > 0 ? "#10b981" : a.pnlPct && a.pnlPct < 0 ? "#ef4444" : "#333" }}>
                {a.pnlPct != null ? `${a.pnlPct > 0 ? "+" : ""}${a.pnlPct.toFixed(2)}%` : "—"}
              </td>
              <td style={{ padding: "4px 8px", color: "#666", maxWidth: "300px", overflow: "hidden", textOverflow: "ellipsis" }}>
                {a.detail ?? ""}
              </td>
            </tr>
          ))}
